...
- 从 Windows 拷来的字典常混入 CRLF 与 BOM，用「工具箱 → 去掉首尾空白」+ 保存为 UTF-8 处理。
...
- 「工具箱」写出的新字典默认放在 `~/.local/share/seclists-gui/output/`。
- 行数统计缓存在 `~/.local/share/seclists-gui/linecounts.json`，删除即可重建。
- 左侧双击文件 = 预览；右键 = 复制各种格式的路径。
- 命令卡片上的「▶ 在终端运行」会新开终端执行该命令（执行前会要求确认）。

## 7. Linux / WSL 小贴士
- 装依赖：Debian/Ubuntu/Kali 用 `sudo apt install python3-tk`；Arch 用 `pacman -S tk`；Fedora 用 `dnf install python3-tkinter`。
- 仓库位置：apt 安装的 SecLists 在 `/usr/share/seclists`；自己 clone 的常见于 `~/SecLists`。界面顶部「路径风格」选 Kali 时，命令里就是 `/usr/share/seclists/...`。
- 中文显示成方框说明缺中文字体：Debian 系装 `fonts-noto-cjk` 或 `fonts-wqy-microhei`。启动时加 `--doctor` 可以自检。
- WSL：需要 WSLg（Windows 11 或已更新的 Windows 10）才能弹窗；也可以用 X server 并 `export DISPLAY=:0`。
- WSL 访问 Windows 上的仓库是 `/mnt/c/...`、`/mnt/e/...`：此时「路径风格」选「本机路径」得到的就是这种路径；要粘贴到 Kali 上跑就切到 Kali 风格。
- 剪贴板由本程序持有：复制后请在本程序运行期间粘贴，关窗后 X11 剪贴板内容会失效。
"""
...
def render_path(rel: str, style: str, root: str) -> str:
    ...
    return f"{base}/{rel_posix.lstrip('/')}" if base else rel_posix
