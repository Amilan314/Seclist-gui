# -*- coding: utf-8 -*-
"""SecLists 助手主窗口：本地 Tkinter 图形界面，不监听端口、不联网。"""
...
STYLE_LABELS = [("本机路径（Windows 绝对路径）", "local"),
...
class SecListsApp(tk.Tk):
    def __init__(self, root_dir: str, data_dir: str):
        ...
        # 本模块里那些常量是导入时的副本，建控件前必须同步一次，否则会用到不存在的字体族。
        sync_fonts(globals())
    ...
    def _build_body(self):
        ...
        for label, fn in [("👀 预览此文件", self._nav_action_preview),
                          ...
                          ("📝 用记事本打开", self._nav_action_notepad),
    ...
    def _build_preview_tab(self):
        ...
        for label, fn in [("📋 复制路径", lambda: self._copy_current_path("local")),
                          ...
                          ("📝 用记事本打开", lambda: self._notepad((self.current_entry or self._dummy()).path)),
    ...
    def _reveal(self, path: str):
        if not path or not os.path.exists(path):
            ...
            return
        try:
            # 列表形式避免 shell 引号问题；explorer 接受 /select,<path> 这种两段式参数
            subprocess.Popen(["explorer", "/select,", os.path.normpath(path)])
        except Exception:  # noqa: BLE001
            try:
                os.startfile(os.path.dirname(os.path.normpath(path)))
            except Exception as exc:  # noqa: BLE001
