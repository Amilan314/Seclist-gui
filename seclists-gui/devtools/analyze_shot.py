...
def main() -> int:
    ...
    shots = sorted(f for f in os.listdir(data_dir) if f.startswith("tab") and f.endswith(".png"))
    if not shots:
        print("没有找到截图，请先运行 devtools/capture_tabs.ps1")
        return 1
    results = [analyze(os.path.join(data_dir, s)) for s in shots]
    print("\n" + "=" * 60)
